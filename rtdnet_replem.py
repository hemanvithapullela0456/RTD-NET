"""
rtdnet_replem.py  —  RTD-Net Slim + LiteASPP + RepLEM
=======================================================
Experiment 2: Adds structural reparameterization inside LEM branches,
on top of Experiment 1 (LiteASPP already included).

RepLEM: what changes inside LEM
---------------------------------
Original LEM branch:
    ConvBNSiLU(mid_ch, br_ch, 1) → ConvBNSiLU(br_ch, br_ch, 3)

RepLEM branch:
    ConvBNSiLU(mid_ch, br_ch, 1) → Rep3x3(br_ch)

Rep3x3 has THREE parallel paths during TRAINING:
    path A: 3×3 conv + BN
    path B: 1×1 conv + BN   (pad to 3×3 at merge time)
    path C: identity  + BN  (only when in_ch == out_ch, which is always true
                              here since br_ch → br_ch)
All three outputs are summed.  After training, call model.reparameterize()
to fuse everything into a single 3×3 conv — ZERO inference overhead.

Why this helps for aerial imagery:
    The three paths force diverse gradient signals into the 3×3 kernel:
    the identity path prevents feature collapse (acts like residual), the
    1×1 path sharpens channel mixing without spatial correlation, and the
    3×3 handles full spatial patterns.  The merged kernel has been shown
    to learn more shape-sensitive (less texture-biased) representations —
    important for AID where scene layout matters more than texture.

Parameter overhead during training: ~+0 inference (fused away).
During training: +2 × (br_ch²) extra params per branch.  For base_ch=32,
br_ch=1 → only 48 branches × 2 extra params = negligible.
"""
...
# ─── NEW: Rep3x3 structural reparameterization block ─────────────────────────

class Rep3x3(nn.Module):
    """
    Structural Reparameterization 3×3 conv.
    Equivalent to a single 3×3 conv at inference; richer gradients at train.

    After training call model.reparameterize() to fuse.
    """
    ...
    def reparameterize(self):
        """Fuse all branches into one 3×3 conv.  Call once, after training."""
        if self._fused:
            return
        k3, b3 = self._fuse_bn(self.conv3, self.bn3)
        k1, b1 = self._fuse_bn_pad(self.conv1, self.bn1)  # 1×1 → padded 3×3
    ...
    def _fuse_bn_pad(conv, bn):
        """Pad 1×1 kernel to 3×3 center then fuse BN."""
...
class RepLEM(nn.Module):
    """
    LEM with Rep3x3 inside each branch.
    Identical interface to LEM; swap without changing any other code.
    """
    ...
    def __init__(self, in_ch, out_ch=None, C=16):
        ...
        self.branches = nn.ModuleList([
            nn.Sequential(ConvBNSiLU(mid_ch, br_ch, 1),
                          Rep3x3(br_ch))          # ← Rep3x3 replaces 3×3 ConvBNSiLU
            for _ in range(C)
        ])
...
class RTDNetRepLEM(nn.Module):
    """
    RTD-Net Slim + LiteASPP + RepLEM.
    Changes vs rtdnet_slim.py:
        SPPFSlim  →  LiteASPP    (experiment 1)
        LEM       →  RepLEM      (experiment 2, this file)
    """
    ...
    def __init__(self, num_classes=30, base_ch=32, num_heads=4,
        ...
        self.lem1  = RepLEM(b*2,  b*2,  C=C)    # ← RepLEM
        self.conv3 = ConvBNSiLU(b*2, b*4,  3, stride=2)
        self.lem2  = RepLEM(b*4,  b*4,  C=C)    # ← RepLEM
        self.conv4 = ConvBNSiLU(b*4, b*8,  3, stride=2)
        self.lem3  = RepLEM(b*8,  b*8,  C=C)    # ← RepLEM
        ...
        self.sppf  = LiteASPP(b*16, b*16)       # ← LiteASPP
    ...
    def reparameterize(self):
        """Call after training to fuse Rep3x3 branches → zero inference cost."""
        for m in self.modules():
            if isinstance(m, Rep3x3):
                m.reparameterize()
        print('Rep3x3 branches fused.  Model is now inference-equivalent.')
...
if __name__ == '__main__':
    ...
    model  = RTDNetRepLEM(num_classes=30).to(device)
    total, _ = model.count_parameters()
    print(f'RTDNet-RepLEM  train params: {total:,}  ({total/1e6:.3f} M)')
