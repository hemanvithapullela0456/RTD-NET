"""
rtdnet_v3.py  —  RTD-Net V3
============================
One change over rtdnet_v2.py:
    single ECTBPlus  →  two stacked ECTBPlus blocks

Everything else is byte-for-byte identical to rtdnet_v2.py.

WHY TWO ECTBPlus BLOCKS
------------------------
Your V2 training curve (95.16% best, epoch 297/300) shows val accuracy
...
----------
V2   : ~3.29M
V3   : ~3.29M + 198K (one more ECTBPlus at mid_ch=C//2=256) = ~3.49M

Note: ECTBPlus uses mid_ch=C//2 (restored in V2). Each block costs
~198K params. Adding one more is cheap relative to the expected gain.
...
# ─────────────────────────────────────────────────────────────────────────────
# CMHSA — identical to rtdnet_v2.py
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# ECTBPlus — identical to rtdnet_v2.py (mid_ch = C//2, DW positional bias)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# RTDNetV3 — V2 + double ECTBPlus
# ─────────────────────────────────────────────────────────────────────────────

class RTDNetV3(nn.Module):
    """
    RTD-Net V3  —  ~3.49M parameters.
    ...
    Stack vs V2:
        ...
        V3 (this) : ectb1 + ectb2  (two stacked ECTBPlus)   ← ONLY CHANGE
...
if __name__ == '__main__':
    ...
    model  = RTDNetV3(num_classes=30, base_ch=32).to(device)
    total, _ = model.count_parameters()

    print(f'V2 (single ECTBPlus) : ~{V2_PARAMS:,}  ({V2_PARAMS/1e6:.3f} M)')
    print(f'V3 (double ECTBPlus) :  {total:,}  ({total/1e6:.3f} M)')
