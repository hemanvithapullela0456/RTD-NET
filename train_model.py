"""
train_all.py - CASA-only training script for RTD-Net classifier

This script follows the same training flow and defaults as train.py,
but trains only the CASA model.

Usage:
    python train_casa.py --data data/aid --dataset aid
    python train_all.py --data data/nwpu --dataset nwpu --num_classes 45
"""
...
from model import RTDNetClassifier as MODEL
...
def setup_logger(save_dir, name):
    ...
    # Clear old handlers to avoid duplicate logs when reusing logger names.
    logger.handlers = []
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="RTD-Net CASA Training")
    ...
    parser.add_argument("--lr_steps", nargs="+", type=int,
                        default=[100, 200],
                        help="Epochs at which LR is reduced by x0.1")
    ...
    # ---- Save directory ----
    run_name = f"{args.dataset}_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    base_save_dir = Path(args.model_save_dir) / run_name
    base_save_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logger(base_save_dir, "casa_train")
    ...
    # Save class mapping at run root
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history["val_top5"].append(round(val_acc5, 2))
